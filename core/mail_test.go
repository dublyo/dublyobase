...
func TestBuildAuthTokenEmail(t *testing.T) {
	...
	msg, err := BuildAuthTokenEmail(cfg, "verify_email", "demo", "User@Example.COM", "dbo_verify_token")
	if err != nil {
		t.Fatal(err)
	}
	if msg.To != "user@example.com" || msg.Subject == "" || !strings.Contains(msg.Text, "dbo_verify_token") {
		t.Fatalf("bad verify email: %+v", msg)
	}
	if !strings.Contains(msg.Text, "https://app.example.com/base/auth/verify?") ||
	...
	msg, err = BuildAuthTokenEmail(cfg, "password_reset", "demo", "user@example.com", "dbo_reset_token")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(msg.Text, "/auth/reset-password?") || !strings.Contains(msg.Text, "dbo_reset_token") {
