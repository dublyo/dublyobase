...
func NewServer(app *core.App) *http.Server {
	...
	mux.Handle("GET /admin/api/projects/{slug}/collections/export", s.requireAdmin(http.HandlerFunc(s.adminExportCollections)))
	mux.Handle("POST /admin/api/projects/{slug}/collections/import", s.requireAdmin(http.HandlerFunc(s.adminImportCollections)))
	mux.Handle("POST /admin/api/projects/{slug}/sql", s.requireAdmin(http.HandlerFunc(s.adminRunSQL)))
	mux.Handle("GET /admin/api/audit-log", s.requireAdmin(http.HandlerFunc(s.adminListAuditLog)))
	mux.Handle("GET /admin/api/settings", s.requireAdmin(http.HandlerFunc(s.adminGetSettings)))
	mux.Handle("PUT /admin/api/settings/smtp", s.requireAdmin(http.HandlerFunc(s.adminUpdateSMTPSettings)))
